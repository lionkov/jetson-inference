...
void cameraThread(CameraImpl *c) {
	...
	int fnum = 0;
	void *cpuframe, *gpuframe, *cpurgba, *gpurgba;

	if (!cudaAllocMapped(&cpuframe, &gpuframe, c->width*c->height*3)) {
		printf("Error: can't alloc GPU data\n");
		exit(-1);
	}

	if (!cudaAllocMapped(&cpurgba, &gpurgba, c->width*c->height*4*sizeof(float))) {
		printf("Error: can't alloc GPU data\n");
		exit(-1);
	}
	...
	while (true) {
		...
		Mat gframe(frame.size(), CV_8UC3, cpuframe);
		cvtColor(frame, gframe, CV_BGR2RGB);

		if( CUDA_FAILED(cudaRGB8ToRGBA32((uchar3*) gpuframe, (float4*) gpurgba, c->width, c->height))) {
			printf("cudaRGB8ToRGBA32 failed\n");
			exit(-1);
		}
		
		const int numDetections = net->Detect((float *) gpurgba, c->width, c->height, &detections, overlayFlags);
		if( numDetections > 0 )
		{
...
#if 1
				rectangle(frame, Point(detections[n].Left, detections[n].Top), Point(detections[n].Right, detections[n].Bottom), Scalar(0, 0, 255));
				putText(frame, net->GetClassDesc(detections[n].ClassID), Point(detections[n].Left, detections[n].Bottom), FONT_HERSHEY_SIMPLEX, 1, Scalar(0, 0, 255), 2);
#endif
			}
		}	
